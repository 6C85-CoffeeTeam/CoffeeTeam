...
const GlobalMap = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  let { GlobalMapDataset = [] } = $$props;
  const projection = d3.geoNaturalEarth1();
  const path = d3.geoPath(projection);
  let recorded_mouse_position = { x: 0, y: 0 };
  let hovered = -1;
  const colorScale = d3.scaleLinear().domain([0, 58211]).range(["white", "brown"]);
  if ($$props.GlobalMapDataset === void 0 && $$bindings.GlobalMapDataset && GlobalMapDataset !== void 0)
    $$bindings.GlobalMapDataset(GlobalMapDataset);
  $$result.css.add(css$2);
  return `<div class="visualization"><svg width="1000" , height="600">${each(GlobalMapDataset, (data, index) => {
...
const NorthernTriangleClimate = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  let { NorthernTriangleClimateDataset = [] } = $$props;
  const projection = d3.geoNaturalEarth1();
  const path = d3.geoPath(projection);
  let recorded_mouse_position = { x: 0, y: 0 };
  let hovered = -1;
  const colorScale = d3.scaleLinear().domain([0, 58211]).range(["white", "brown"]);
  if ($$props.NorthernTriangleClimateDataset === void 0 && $$bindings.NorthernTriangleClimateDataset && NorthernTriangleClimateDataset !== void 0)
    $$bindings.NorthernTriangleClimateDataset(NorthernTriangleClimateDataset);
  $$result.css.add(css$1);
  return `<div class="visualization"><svg width="1000" , height="600">${each(NorthernTriangleClimateDataset, (data, index) => {
    return `<path${add_attribute("d", path(data), 0)}${add_attribute(
      "fill",
      index === hovered ? "green" : colorScale(data["properties"]["coffee"]),
      0
...
const App = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  d3.json("https://raw.githubusercontent.com/austinjstromme/CoffeeTeam/main/coffee-app/data/northerntriangleclimate.geojson").then((data) => {
  ...
  return `<main><title>CoffeeApp
    ...
    <h1 class="svelte-12200kg">The coffee belt: global coffee production is concentrated in the tropics
      ...
      ${validate_component(GlobalMap, "GlobalMap").$$render($$result, { GlobalMapDataset }, {}, {})}
      <h1 class="svelte-12200kg">Climate and coffee production within the northern triangle
      </h1>
      ${validate_component(NorthernTriangleClimate, "NorthernTriangleClimate").$$render($$result, { NorthernTriangleClimateDataset }, {}, {})}
  </main>`;
});
